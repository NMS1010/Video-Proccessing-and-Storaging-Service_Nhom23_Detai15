...
@pytest.mark.parametrize('projects', [({'file': 'sample_0.mp4', 'duplicate': False},)], indirect=True)
def test_get_raw_video_404_not_object_id(test_app, client, projects):
    project = projects[0]
    
    with test_app.test_request_context():
        
        url = url_for('projects.get_raw_video', project_id="definitely_not_object_id")
        resp = client.get(url)
        assert resp.status == '404 NOT FOUND'

@pytest.mark.parametrize('projects', [({'file': 'sample_0.mp4', 'duplicate': False},)], indirect=True)
def test_get_raw_video_404_wrong_id(test_app, client, projects):
    project = projects[0]
